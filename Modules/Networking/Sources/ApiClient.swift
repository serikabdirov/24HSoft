...
public class ApiClient {
    ...
    public func upload<S: ResponseSerializer>(
        _ target: TargetType,
        responseSerializer: S
    ) async throws -> DataResponse<S.SerializedObject, AFError> {
        await session.upload(multipartFormData: target.multipartFormData, with: target)
            .serializingResponse(using: responseSerializer, automaticallyCancelling: true)
            .response
    }
    ...
    /// create URLRequest fromTargetType with session adapters
    public func createRequest(target: TargetType) -> URLRequest? {
        guard
            let request = try? target.asURLRequest(),
            let adapter = session.interceptor else {
            return nil
        }
    ...
    /// create URLRequest from URL or URL string with session adapters
    public func adaptRequest(urlRequest: URLRequest) -> URLRequest? {
        guard
            let adapter = session.interceptor else {
            return urlRequest
        }
